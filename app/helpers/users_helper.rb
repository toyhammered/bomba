  ...
  def profile_friendship_button(user)
    ...
    if friendships.empty? && pending_friendships.empty?
      ...
      content_tag(:div, class: "requested-friends-helper") do
        (link_to "<i class='fa fa-2x fa-check'></i>".html_safe, accept_pending_friendship_path(pending_friendships.take.id), method: :post) +
        (link_to "<i class='fa fa-2x fa-times'></i>".html_safe, pending_friendship_path(pending_friendships.take.id), method: :delete).html_safe
      end
    end
